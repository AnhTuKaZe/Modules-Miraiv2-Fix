...
----𝕃𝕒𝕫𝕪_ℙ𝕣𝕠𝕛𝕖𝕔𝕥----`,attachment: fs.createReadStream(__dirname + "/cache/stk.gif")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/stk.gif")); 
      return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname+"/cache/stk.gif")).on("close",() => callback());
   };
